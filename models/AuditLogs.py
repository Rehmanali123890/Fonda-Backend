...
class AuditLogs():
  ...
  @classmethod
  def get_audit_logs(cls, request):
    try:
      ...
      if request.args.get("startDate") and request.args.get("endDate"):
        ...
        conditions.append(f"""
          convert_tz(
            eventdatetime, 
            '+00:00', 
            COALESCE((SELECT merchants.timezone FROM merchants where merchants.id = auditlogs.merchantid LIMIT 1), 'UTC')
      ...
      cursor.execute(f"""
        SELECT id, userid, username, merchantid, merchantname, 
          ...
          @timezone := (SELECT @timezone := merchants.timezone FROM merchants where merchants.id = auditlogs.merchantid LIMIT 1) as timezone,
  ...
  @classmethod
  def post_audit_logs(cls, userid, username, merchantid, merchantname, eventname=None, eventdetails=None, payoutid=None, subscriptionid=None):
    try:
      connection, cursor = get_db_connection()

      log_id = uuid.uuid4()
      data = (log_id, userid, username, merchantid, merchantname, eventname, eventdetails, payoutid, subscriptionid)
      cursor.execute("""INSERT INTO auditlogs 
